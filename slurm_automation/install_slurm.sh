...
download_files() {
    ...
    sudo wget -P $user_home_dir https://download.schedmd.com/slurm/slurm-$VER.tar.bz2
    sudo wget -P $user_home_dir https://github.com/dun/munge/releases/download/munge-$MUNGE_VER/munge-$MUNGE_VER.tar.xz
    sudo wget -P $user_home_dir https://download.open-mpi.org/release/hwloc/v2.11/hwloc-$HWLOC_VER.tar.bz2
}
...
build_munge() {
    ...
    # Added --localstatedir=/var to fix the socket location natively
...
create_munge_key() {
    # Sometimes mungekey is in /usr/sbin due to our --prefix
...
set_munge_permissions() {
    ...
    # Automated safety net for the socket issue, just in case
...
start_munge_service() {
    echo "--> Starting Munge"
    # Create a systemd service file for Munge if it wasn't installed
...
EOF'
    ...
    sleep 2 # Give Munge a moment to establish the socket
}
...
add_additional_configurations() {
    # Dynamically inject the node configuration
...
start_slurm_services() {
    ...
    # Copy systemd files to universal /etc/systemd/system path
    ...
    while ! systemctl is-active --quiet slurmctld; do sleep 1; done
